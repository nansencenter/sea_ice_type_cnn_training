#this script allows to perform the training and to calculate the metrics for the networks with AMSR2 data and with a new structure
...
model = create_model()
#callbacks
mc = tf.keras.callbacks.ModelCheckpoint(filepath='hugo_model_continous_amsr2_hyper_all50_nopatience', 
                                        monitor='val_loss',
                                        verbose=1, 
                                        save_best_only=True,
                                        mode='min')
...
plt.savefig("history_hugo_continous_amsrs2_hyper_all50_nopatience")
plt.show()
...
plt.savefig('rmse_matrix_hugo_continous_amsr2_hyper_all50_nopatience')
plt.show()

fig3 = plt.figure()
plt.imshow(pearson_matrix)
plt.savefig('pearson_matrix_hugo_continous_amsr2_hyper_all50_nopatience')
plt.show()
