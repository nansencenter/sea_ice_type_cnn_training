# this script is used to train and calculate the metrics for the sod network foe 17 classes
...
model = create_model()
#callbacks
mc = tf.keras.callbacks.ModelCheckpoint(filepath='model_sod_50_17_nopatience', 
                                        monitor='val_loss',
                                        verbose=1, 
                                        save_best_only=True,
                                        mode='min')
...
plt.savefig("history_model_sod_50_17_nopatience")
plt.show()
...
plt.clf()
plt.figure(figsize=(18,18))
fx = sebrn.heatmap(rrmse_matrix, annot=True, cmap='Blues', fmt=".3f", cbar=False,  vmax=0.01)
