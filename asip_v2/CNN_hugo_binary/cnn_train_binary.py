#this script allows the training and the calculation of the metrics for the neural network of the binary method
...
def create_model():
    """
    Create sequential CNN with convolutional and dense layers for binary netwoek
    """
...
model = create_model()
#callbacks
mc = tf.keras.callbacks.ModelCheckpoint(filepath='hugo_model_binary', 
                                        monitor='val_loss',
                                        verbose=1, 
                                        save_best_only=True,
                                        mode='min')
...
plt.savefig("history_hugo_binary")
plt.show()
...
fig1 = plt.figure()
plt.imshow(cm)
plt.savefig('confusion_matrix_hugo_binary')
plt.show()

#pearson and rmse matrice
y_pred = model.predict(validation_generator)
y_val = np.vstack([vg[1] for vg in validation_generator])
...
plt.savefig('rmse_matrix_hugo_binary')
plt.show()

fig3 = plt.figure()
plt.imshow(pearson_matrix)
plt.savefig('pearson_matrix_hugo_binary')
plt.show()
