# this script is used to train and calculate the metrics for the continuous method network

idir = '/tf/data/hugo_continous/output_preprocessed_continous/output/'
input_dir_json = '/tf/data/hugo_continous/output_preprocessed_continous/check/'
...
model = create_model()
#callbacks
mc = tf.keras.callbacks.ModelCheckpoint(filepath='hugo_model_continous', 
                                        monitor='val_loss',
                                        verbose=1, 
                                        save_best_only=True,
                                        mode='min')
...
plt.savefig("history_hugo_continous")
plt.show()
...
plt.savefig('rmse_matrix_hugo_continous')
plt.show()

fig3 = plt.figure()
plt.imshow(pearson_matrix)
plt.savefig('pearson_matrix_hugo_continous')
plt.show()
